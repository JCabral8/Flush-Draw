...
function usable(sim: SimState, id: ClubId): boolean {
  ...
  if (onGreen && (spec.kind === 'swing' || id === 'sevenIron')) return false
      ...
      {sim.config.bag.map((id) => {
        ...
        return (
          <button
            ...
            disabled={!ok && !armed}
            aria-pressed={armed}
            aria-label={`${t(`club.${id}.name` as StringKey)}: ${t(`club.${id}.fx` as StringKey)}`}
            onClick={() => toggleClub(id)}
          >
            <span className="club-name">{t(`club.${id}.name` as StringKey)}</span>
            {left !== null && (
